function Diagnosa() {
  ...
  if (Object.keys(presentasePenyakit).length > 0) {
    resultDiv.innerHTML += '<ul>';
    for (let penyakit in presentasePenyakit) {
      resultDiv.innerHTML += '<li>' + penyakit + ': ' + presentasePenyakit[penyakit] + '%</li>';
    }
    resultDiv.innerHTML += '</ul>';
    // resultDiv.innerHTML += '<p>Total Presentase: ' + totalPresentase.toFixed(2) + '%</p>';
  } else {
    resultDiv.innerHTML += '<p>Tidak ada penyakit yang mungkin terkena berdasarkan gejala yang dipilih.</p>';
  }
}
